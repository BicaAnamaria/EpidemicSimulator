...
{ 
  ...
  if(type > 1) {
    ...
    if(type == 2) {
      ...
      leg.xy = c(0.9, max(p.old, out$I, max(out$Hcum) ) * 0.8) ;
  ...
  plot.sir(out, legend.lbl = lbl, legend.xy = leg.xy, add = add, plot.legend = plot.legend, title = "SIR Hospitalisation Model", ...);
}
